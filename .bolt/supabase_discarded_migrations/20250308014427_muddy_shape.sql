/*
  ...
  3. Functions
    - process_order_total: Automatic total calculation
    - process_order_tickets: Ticket generation
    - process_order_email: Email notifications
...
-- Create functions
CREATE OR REPLACE FUNCTION process_order_total()
RETURNS TRIGGER AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION process_order_tickets()
RETURNS TRIGGER AS $$
DECLARE
  v_ticket_type RECORD;
  v_qty INTEGER;
...
CREATE OR REPLACE FUNCTION process_order_email()
RETURNS TRIGGER AS $$
BEGIN
  PERFORM http_post(
...
-- Grant permissions
GRANT EXECUTE ON FUNCTION process_order_total TO authenticated, public;
GRANT EXECUTE ON FUNCTION process_order_tickets TO authenticated, public;
GRANT EXECUTE ON FUNCTION process_order_email TO authenticated, public;