        ...
        <div className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg z-[101]">
                ...
                <div className="space-y-4">
                  {ticketTypes.map((ticket) => (
                    selectedTickets[ticket.id] > 0 && (
                      <div key={ticket.id} className="border-b border-[var(--gray-200)] pb-4">
                        <div className="flex justify-between items-start">
                          <div>
                            <h4 className="font-medium text-[var(--gray-900)]">{ticket.name}</h4>
                            <p className="text-sm text-[var(--gray-600)]">{ticket.description}</p>
                          </div>
          ...
          <div className="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6 gap-2">
            <button
              type="button"
              onClick={onConfirm}
              className="inline-flex w-full justify-center rounded-lg bg-[var(--primary-600)] px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-[var(--primary-500)] sm:ml-3 sm:w-auto"
            >
            ...
            <button
              type="button"
              onClick={onClose}
              className="mt-3 inline-flex w-full justify-center rounded-lg bg-white px-3 py-2 text-sm font-semibold text-[var(--gray-900)] shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-[var(--gray-50)] sm:mt-0 sm:w-auto"
            >
