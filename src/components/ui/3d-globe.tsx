  ...
  return (
    <group ref={groupRef} visible={isVisible}>
      ...
      <mesh position={surfacePosition} quaternion={lineQuaternion}>
        ...
        <meshBasicMaterial color={hovered ? '#C68A1F' : '#3D3FE2'} />
      </mesh>
...
function RotatingGlobe({ config, markers, onMarkerClick, onMarkerHover }: RotatingGlobeProps) {
  const earthTexture = useTexture(config.textureUrl);
  ...
  return (
    <Html center>
      <div className="flex shrink-0 flex-col items-center gap-3">
        <span className="inline-block shrink-0 text-sm text-neutral-400">Chargement du globe…</span>
      </div>
    </Html>
  );
}
...
export function Globe3D({
  ...
  return (
    <div className={cn('relative h-full w-full min-h-[240px]', className)}>
      <Canvas
