/*
  # Guest Orders System Update

  1. Functions
    - generate_guest_token(): Generates unique tokens for guest orders
    - validate_guest_order(): Validates guest order data
    - create_guest_order_direct(): Creates guest orders with validation
...
CREATE POLICY "guest_orders_view_policy_v3"
  ON guest_orders
  FOR SELECT
  USING (email = current_user);

CREATE POLICY "guest_orders_insert_policy_v3"
  ON guest_orders
  FOR INSERT
  WITH CHECK (true);