/*
  ...
  1. Changes
    - Implemented bulk ticket creation to prevent duplicates
    - Added transaction safety with row locking
    - Improved error handling and validation
    - Added proper quantity checks
    
  2. Functions
    - calculate_order_total: Calculates total price for order
    - create_tickets_for_order: Creates tickets with bulk insert
...
-- Create tickets for order function with bulk insert
...
BEGIN
  ...
  LOOP
    ...
    -- Get ticket type details and lock row
    ...
    IF NOT FOUND OR v_ticket_type.available < v_quantity::integer THEN
    ...
    -- Insert tickets in bulk using generate_series
    INSERT INTO tickets (
      order_id,
      event_id,
      user_id,
    ...
    SELECT 
      ...
      encode(
        digest(
          NEW.id::text || v_ticket_type_id || s::text || clock_timestamp()::text,
          'sha256'
        ),
        'hex'
      )
    FROM generate_series(1, v_quantity::integer) s;
  END LOOP;
