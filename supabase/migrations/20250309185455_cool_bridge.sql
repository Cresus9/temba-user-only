/*
  ...
  1. Changes
    - Drop and recreate ticket transfer views and policies
    - Update foreign key relationships for sender_id and recipient_id
    - Add proper triggers and functions for transfer handling

  2. Security
    - Enable RLS on ticket_transfers table
    - Add policies for viewing, creating, and updating transfers
    - Ensure proper access control for transfer operations

  3. Notes
    - Drops existing objects first to avoid conflicts
    - Uses CASCADE where necessary to handle dependencies
*/

-- Drop existing objects with CASCADE to handle dependencies
DROP VIEW IF EXISTS ticket_transfer_details CASCADE;
DROP TRIGGER IF EXISTS on_transfer_request_created ON ticket_transfers CASCADE;
DROP TRIGGER IF EXISTS on_transfer_complete ON ticket_transfers CASCADE;
...
DROP CONSTRAINT IF EXISTS ticket_transfers_sender_id_fkey CASCADE,
DROP CONSTRAINT IF EXISTS ticket_transfers_recipient_id_fkey CASCADE;
